...
public final class String
     ...
     * @since JDK1.1
     */
    //传bytes数组,偏移量0,截取量为bytes的length长度,编码格式名称.
     ...
     */
    //不对外提供的构造构造函数,区别于String(char[] value)构造方法,这里属于直接赋值value.性能更好,避免了数组拷贝.不对外提供此函数是为了不打破打破value为不变数组的限制
    ...
    //但当String是Unicode类型时则有区别了。如String str = “/uD835/uDD6B” (即使 'Z' ), length() = 2 ,codePointCount() = 1
     ...
     */
    //比较字源字符串是否与目标对象相等(anObject只允许为字符串类型)
    public boolean equals(Object anObject) {
        //判断是否anObject为String类型,如果不是则直接返回false
        if (anObject instanceof String) {
            ...
            if (n == anotherString.value.length) {
                //第二部取出两个字符串的的value字符数组
                char v1[] = value;
                char v2[] = anotherString.value;
                int i = 0;
                //倒序遍历,挨个字符进行对比,知道循环次数等于源字符串的长度.
                // n--会直接先运行一次n-1的操作.再进入下面的逻辑.所以不会数组越界
                while (n-- != 0) {
                    //当遇到不相等的字符就直接返回false表示源字符串和目标对象不相等
                    if (v1[i] != v2[i])
                        return false;
                    i++;
                }
     ...
     * @since 1.4
     */
    //StringBuffer与源字符串比较.重载了contentEquals(CharSequence cs)方法
    public boolean contentEquals(StringBuffer sb) {
        return contentEquals((CharSequence) sb);
    }

    //StringBuffer和StringBuilder与源字符串比较.相等返回true
    ...
    public boolean contentEquals(CharSequence cs) {
        //StringBuilder和StringBuffer类型都是实现了AbstractStringBuilder抽象类
        if (cs instanceof AbstractStringBuilder) {
            //因为StringBuffer是多线程安全的,所以需要在调用方法时加锁保证在方法进行时StringBuffer对象未被修改
            if (cs instanceof StringBuffer) {
                synchronized (cs) {
                    return nonSyncContentEquals((AbstractStringBuilder) cs);
                }
            } else {
                //StringBuilder直接调用nonSyncContentEquals函数处理
                return nonSyncContentEquals((AbstractStringBuilder) cs);
            }
        }
        //如果是String类型,直接调用equal方法
        if (cs instanceof String) {
            return equals(cs);
        }
        //如果是其他继承CharSequence接口的对象
        //取出源字符串的value字符数组和源字符串的长度
        char v1[] = value;
        int n = v1.length;
        // 继承了CharSequence接口的都会有length方法,比较他们的长度
        if (n != cs.length()) {
            return false;
        }
        // 再用charAt正序弹出各下标的字符进行对比,如果有不等的则返回不相等
        for (int i = 0; i < n; i++) {
            if (v1[i] != cs.charAt(i)) {
                return false;
            }
     ...
     * @see #equals(Object)
     */
    //比较源字符串与目标字符串是否相等.此方法对比equal忽略了大小写
    public boolean equalsIgnoreCase(String anotherString) {
        // 因为! || &&的优先级为 !>&&>|| 所以可缩写为以下代码.但是可读性上还是源码强一些
        //        return (this == anotherString) || (anotherString != null)
        //                && (anotherString.value.length == value.length)
        //                && regionMatches(true, 0, anotherString, 0, value.length);

        //先判断引用地址是否相同.然后判断是否是null字符.接着判断长度,然后regionMatches就是把挨个字符转大写对比再转小写对比
     ...
     * </ul>
     *
     * @param ignoreCase if {@code true}, ignore case when comparing  如果为 true，则比较字符时忽略大小写。
     *                   characters.
     * @param toffset    the starting offset of the subregion in this 此字符串中子区域的起始偏移量。
     *                   string.
     * @param other      the string argument. 字符串参数
     * @param ooffset    the starting offset of the subregion in the string 字符串参数中子区域的起始偏移量。
     *                   argument.
     * @param len        the number of characters to compare. 要比较的字符数
     * @return {@code true} if the specified subregion of this string
    ...
    static int indexOf(char[] source, int sourceOffset, int sourceCount,
                       ...
                       int fromIndex) {
        //需要目标字符串起始位置大于等于传入的源字符串的截取值sourceCount
        if (fromIndex >= sourceCount) {
            //需要目标字符串截取值为0,则返回0,不为0返回-1
        ...
        //如果目标字符串截取值为0,也就是空字符串直接返回查找起始位置fromIndex的值
        if (targetCount == 0) {
            return fromIndex;
        }

        //需要目标字符串第targetOffset元素(即需要查找的第一个字符)
        char first = target[targetOffset];
        //循环最大次数为源字符串的起始偏移值加上 源字符串的截取长度与目标字符串截取长度的差
        //这一步可以直接过滤掉源字符串长度小于目标字符串长度的情况
        int max = sourceOffset + (sourceCount - targetCount);

        //循环从源字符串起始偏移值加上查找起始位置开始.
        for (int i = sourceOffset + fromIndex; i <= max; i++) {
            ...
            //查找剩余相等的字符
            if (i <= max) {
                //上面的循环得出了第一个相等字符的位置i,所以在源字符串中查找第二个相同字符起点为 i+1
                int j = i + 1;
                //循环结束点为要目标字符串的长度-1,后面的targetCount - 1用括号括起来比较容易理解
                int end = j + targetCount - 1;
                //k的起始点为目标字符串第二个点的下标,因为前面已经找到了第一个相同字符,所以下标要从第二个点开始
                //循环进行条件:当前循环中源字符串的下标小于循环结束点且循环中源字符串的j下标字符和目标字符串k下标的字符相等
                //满足条件 j+1 k+1
                // 改写为以下方式可能比较好理解
                //      int k = targetOffset + 1;  //目标字符串的起始下标
                //      while (j < end && source[j] == target[k]) {  //循环结束条件
                //          j++;  //源字符串的j下标+1
                //          k++;  //目标字符串k下标+1
                //      }
                ...
                //如果循环结束后,源字符串的下标j和循环结束点相等,说明源字符串完全包含目标字符串
                if (j == end) {
                    //第一次出现的点就是第一次匹配到的字符下标减去目标字符串的偏移值
     ...
     */
    //连接字符串(底层通过new StringBuilder 实现,推荐复杂的String拼接直接使用StringBuilder)
    public String concat(String str) {
        ...
        //追加str字符串数的value数组到当前字符串的起始位置为len的位置.
        str.getChars(buf, len);
        //直接把buf字符数组赋值给当前字符串对象的value
        //总共经历了两次数组拷贝.
     ...
     * @since 1.5
     */
    //当前字符串中是否包含s字符(CharSequence为接口,s 可传入StringBuilder,StringBuffer,String)
    public boolean contains(CharSequence s) {
        //调用s类型对应的toString方法返回的字符串传入indexOf方法,大于等于0为包含,返回true
